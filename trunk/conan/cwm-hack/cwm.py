...
#            _equals = string.find(arg, "=")
        ...
        if option_format == "rdf":
            _outSink = toXML.ToRDF(stream, _outURI, base=option_baseURI, 						flags=option_flags["rdf"])
        elif option_format == "n3" or option_format == "sparql":
            _outSink = notation3.ToN3(stream.write, base=option_baseURI,
            ...
            if option_pipe:
                ...
                _outSink.backing = notation3.ToN3(stream.write,
                        base=option_baseURI, quiet=option_quiet,
                        flags=option_flags["n3"])

                #  hm.  why does TimBL use stream.write, above?  performance at the                
        else:
        ...
        if diag.tracking: 
            myReason = BecauseOfCommandLine(`fargs`)
